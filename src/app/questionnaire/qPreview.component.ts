...
@Component({
    templateUrl: './qPreview.component.html',
    styleUrls: ['./qPreview.component.css']
})
...
export class QPreviewComponent implements OnInit {
    ...
    // @HostBinding('style.display')   mDisplay = 'block';
    ...
    ngOnInit() {
        ...
        var minutes = 60 * 60,
        mDisplay = document.querySelector('#Mtime');
       this.startTimerMobile(minutes, mDisplay);
            ...
            if (response.status === 'success') {
                ...
                for (var i = 1; i <= this.questions.length; i++) {
                    ...
                    if (innerArray.length ==10) {
                        this.mobileMainArray.push(innerArray);
                        innerArray = new Array;
                    }
                    if (i == this.questions.length && !(innerArray.length <= 0)) {
                        this.mobileMainArray.push(innerArray);
    ...
    startTimerMobile(duration, mDisplay) {
        var start = Date.now(),
            diff,
            minutes,
            seconds;
        function timer() {
            ...
            mDisplay.textContent = minutes + " : " + seconds;
